...
public class TeamTablesLoaderTest extends CassandraIntegration {
    ...
    public void whenLoadingTeamsWithBatch_thenAvailableOnRetrieval() throws Exception {
        ...
        TeamRosterByYear bullsTeamGasol = new TeamRosterByYear(2016, "Chicago Bulls", UUID.randomUUID(), "Pau Gasol", FORWARD_CENTER.name());
        TeamRosterByYear bullsTeamRose = new TeamRosterByYear(2016, "Chicago Bulls", UUID.randomUUID(), "Derrick Rose", POINT_GUARD.name());
        TeamRosterByYear bullsTeamButler = new TeamRosterByYear(2016, "Chicago Bulls", UUID.randomUUID(), "Jimmy Butler", SHOOTING_GUARD.name());

        List<TeamRosterByYear> bullsRoster = Arrays.asList(bullsTeamButler, bullsTeamGasol, bullsTeamRose);
        ...
        List<TeamRosterByYear> insertedTeamPlayers = teamRosterByYearRepository.findTeamRosterByYear(2016, "Chicago Bulls");
